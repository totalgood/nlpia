...
def generate_download_mccauley():
    ...
    response = requests.get('http://jmcauley.ucsd.edu/data/amazon/', allow_redirects=True, timeout=5)
    urls_product_review = [m[0] for m in regexes.cre_url_popular.findall(response.text) if m[0].lower().endswith('.json.gz')]

    response = requests.get('http://jmcauley.ucsd.edu/data/amazon/qa/', allow_redirects=True, timeout=5)
